...
export function AuditComplimentary() {
  const { dailyData } = useApp()
  const { complimentaryTotal, complimentaryDetails, complimentaryDishes } = dailyData.audit
  const sorted = [...complimentaryDetails].sort((a, b) => b.percentage - a.percentage)
  const sortedDishes = [...complimentaryDishes].sort((a, b) => b.percentage - a.percentage)
  const [expanded, setExpanded] = useState<string | null>(null)

  return (
    <div className="page-container bg-page-body">
      <DrillDownHeader title="赠菜" />
      ...
      <div className="card-elevated p-4 mb-4">
        ...
        <div className="flex items-baseline justify-between mb-3">
          <h2 className="card-title">赠菜金额</h2>
          <span className="metric-secondary">¥{complimentaryTotal}</span>
        </div>
