...
export const VoiceDianping = () => {
  const { data } = useData();
  const platform = data.voice.platforms.find(p => p.platform === '大众点评') || data.voice.platforms[1];

  const reviews = data.voice.reviews.filter(r => r.platform === '大众点评');

  return (
    <div className="min-h-screen bg-bg-page">
      <DrillDownHeader title="大众点评口碑" subtitle={data.date} />
              ...
              <div key={score} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-6 bg-gray-100 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: score >= 5 ? '60%' : score === 4 ? '28%' : score === 3 ? '8%' : score === 2 ? '3%' : '1%' }}
                  />
                </div>
                <div className="w-12 text-right text-body-sm font-medium">
                  {score >= 5 ? '60%' : score === 4 ? '28%' : score === 3 ? '8%' : score === 2 ? '3%' : '1%'}
                </div>
              </div>
            ))}
          </div>
